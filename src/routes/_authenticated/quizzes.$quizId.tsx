...
type Result = { correct: boolean; correct_index: number; chosen_index: number; explanation: string | null };
...
function QuizPage() {
  ...
  const [outcome, setOutcome] = useState<{ score: number; total: number; xpEarned: number; results: Result[] } | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ["quiz", quizId],
    queryFn: async () => {
      const { data: quiz, error } = await supabase
        .from("quizzes").select("id, title, language_code, level").eq("id", quizId).maybeSingle();
      if (error) throw error;
      if (!quiz) return null;
      const { data: questions, error: qErr } = await supabase
        .from("quiz_questions").select("id, prompt, options, sort_order")
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = (await submitFn({ data: { quiz_id: quizId, answers: ordered } })) as typeof outcome;
      setOutcome(res);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        <Link to="/quizzes" className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        ...
        {isLoading ? (
          ...
          <div>
            <div className="rounded-2xl border border-border bg-card p-8 text-center shadow-soft">
              ...
              <h2 className="mt-4 text-2xl font-bold">{outcome.score} / {outcome.total} correct</h2>
              ...
              {data.questions.map((q, i) => {
                ...
                return (
                  <div key={q.id} className="rounded-xl border border-border bg-card p-4">
                    ...
                    <p className={cn("mt-2 text-sm", r.correct ? "text-primary" : "text-destructive")}>
                      {r.correct ? "Correct" : `Your answer: ${q.options[r.chosen_index] ?? "—"}`}
                    </p>
                    {!r.correct && <p className="text-sm text-primary">Answer: {q.options[r.correct_index]}</p>}
                    {r.explanation && <p className="mt-1 text-sm text-muted-foreground">{r.explanation}</p>}
                  </div>
                );
              })}
            </div>
            <Link to="/quizzes"><Button variant="hero" className="mt-6 w-full">Back to quizzes</Button></Link>
          </div>
        ) : (
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{data.quiz.title}</h1>
            <p className="text-sm uppercase text-muted-foreground">{data.quiz.language_code} · {data.quiz.level}</p>
                    ...
                    {q.options.map((opt, oi) => (
                      <button
                        ...
                        className={cn(
                          ...
                          answers[i] === oi ? "border-primary bg-primary/10 font-medium text-primary" : "border-border hover:bg-muted",
                        )}
                      >
                        {opt}
                      </button>
            ...
            <Button variant="hero" className="mt-6 w-full" onClick={handleSubmit} disabled={submitting}>
